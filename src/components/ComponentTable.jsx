...
function MoveBtn({ label, tone, onClick, className = '', children }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center justify-center gap-1.5 rounded-md border border-line bg-surface px-2.5 py-2 text-xs font-medium text-mute transition hover:text-ink sm:py-1.5 ${t.hover} ${className}`}
    >
...
export default function ComponentTable({ rows, onInward, onOutward, onReturn, onHistory, onEdit, onDelete }) {
  ...
  return (
    <>
      ...
      <div className="hidden max-h-[75vh] overflow-auto rounded-lg bg-surface ring-1 ring-line md:block">
        <table className="w-full min-w-[1000px] border-collapse text-sm">
          <thead className="sticky top-0 z-10 border-b border-line bg-surface2/95 backdrop-blur">
            <tr>
              ...
              <th className={`${th} hidden lg:table-cell`}>Label</th>
            ...
            {rows.map((c) => {
              ...
              return (
                <tr key={c.id} className="transition hover:bg-surface2/70">
                  ...
                  <td className={`${td} hidden max-w-[180px] lg:table-cell`}>
